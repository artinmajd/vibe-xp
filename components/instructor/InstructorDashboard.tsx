...
import MessagesTab from "@/components/instructor/MessagesTab";
...
type Tab = "submissions" | "teams" | "achievements" | "session" | "leaderboard" | "messages";
...
export default function InstructorDashboard({ pending, approved, teams, teamlessStudents, cohortStudents, sessions, activeSession, nextAchievement, lastUnlockedAchievement, sessionAchievements, chatEnabled: initialChatEnabled, cohort }: Props) {
  ...
  function resetLimitsForm() {
    ...
    setLimitMaxTeams(cohort.max_teams);
    setLimitMaxMembers(cohort.max_team_members);
    setLimitsError(null);
  }
  ...
  return (
    <main className="min-h-screen bg-zinc-950 text-white">
      ...
      <div className="border-b border-zinc-800 px-6 flex gap-1">
        {(["submissions", "teams", "achievements", "session", "messages", "leaderboard"] as Tab[]).map((t) => (
          <button
            key={t}
            onClick={() => setTab(t)}
      ...
      <div className="px-6 py-6 max-w-5xl mx-auto">
        ...
        {tab === "teams" && (
          <div className="flex flex-col gap-6">
            ...
            {!settingLimits ? (
              <button
                onClick={() => setSettingLimits(true)}
                className="cursor-pointer self-start flex items-center gap-2 text-xs font-semibold bg-indigo-950/30 hover:bg-indigo-950/50 text-indigo-200 border-2 border-indigo-500/40 px-4 py-2.5 rounded-lg transition-colors"
              >
              ...
              <div className="rounded-xl p-5 border-2 border-indigo-500/40 bg-indigo-950/30 shadow-[0_0_0_1px_rgba(99,102,241,0.15)]">
                ...
                <div className="flex items-end gap-4 flex-wrap">
                  ...
                  <button
                    onClick={resetLimitsForm}
                    className="cursor-pointer text-indigo-300/70 hover:text-indigo-200 text-xs px-3 py-2.5 transition-colors"
                  >
                    Cancel
                  </button>
                </div>
